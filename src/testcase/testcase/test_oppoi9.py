...
from src.functions.driver_now import qudong_oppo9
...
time.sleep(3)
try:
    driver.find_element_by_id("com.cmcc.cmvideo:id/skip_button").click()
except BaseException as msg:
    print(msg)
#time.sleep(5)
#driver.quit()
picture_time = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))
pathnow = mkdir.mkdir("d:\\tool\\android_auto\\appiumtest\\Picture\\" + time.strftime("%Y-%m-%d-%H-%M",
                                                                                              time.localtime(
                                                                                                  time.time())) + "\\")
print("当前照片存储文件夹为："+pathnow)
